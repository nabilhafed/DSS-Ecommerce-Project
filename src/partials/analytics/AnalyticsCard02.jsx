          ...
          <table className="table-auto w-full dark:text-gray-300">
            <thead className="text-xs uppercase text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-700 rounded-sm">
              <tr>
                ...
                <th className="p-2">Maximax</th>
                <th className="p-2">Maximin</th>
                <th className="p-2">Savage</th>
                <th className="p-2">Laplace</th>
                <th className="p-2">Hurwicz</th>
                <th className="p-2">Bernoulli</th>
              </tr>
            </thead>
                ...
                tableData.categories.map((category) => (
                  <tr key={category}>
                    ...
                    <td className="p-2 text-center">
                      {results.maximax.find((item) => item.category === category)?.result}
                    </td>
                    <td className="p-2 text-center">
                      {results.maximin.find((item) => item.category === category)?.result}
                    </td>
                    <td className="p-2 text-center">
                      {results.savage.find((item) => item.category === category)?.result}
                    </td>
                    <td className="p-2 text-center">
                      {results.laplace.find((item) => item.category === category)?.result}
                    </td>
                    <td className="p-2 text-center">
                      {results.hurwicz.find((item) => item.category === category)?.result}
                    </td>
                    <td className="p-2 text-center">
                      {results.bernoulli.find((item) => item.category === category)?.result}
                    </td>
                  </tr>
                ))}
            </tbody>
