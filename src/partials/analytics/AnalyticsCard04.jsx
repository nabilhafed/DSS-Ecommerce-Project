...
function AnalyticsCard04() {
    ...
    const [results, setResults] = useState({
        ...
        majorityVote: null, // Décision finale globale par vote majoritaire
    });
    ...
    const calculateCriteria = (categories, table, regions) => {
        ...
        const votes = Object.values(finalDecisions).map((decision) => decision.optimalCategory);
        const voteCounts = votes.reduce((acc, category) => {
            acc[category] = (acc[category] || 0) + 1;
            return acc;
        }, {});

        const majorityVote = Object.entries(voteCounts).reduce((max, [category, count]) => {
        ...
        setResults({
            ...
            finalDecisions,
            majorityVote: majorityVote.category, // Catégorie avec le vote majoritaire
        });
    };

    return (
        <div className="flex flex-col col-span-full sm:col-span-12 xl:col-span-6 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
            <div className="px-5 pt-5 ">
                <header className="flex justify-between items-start mb-2">
                    <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-2"> Décision Finale</h2>
                </header>
                <div className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase mb-1">Catégorie Choisie :</div>
                <div className="flex items-start">
                    <div className="text-3xl font-bold text-gray-800 dark:text-gray-100 mr-2">  <strong>{results.majorityVote}</strong></div>
                </div>
            </div>
